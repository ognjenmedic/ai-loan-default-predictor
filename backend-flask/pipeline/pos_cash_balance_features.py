...
def compute_pos_cash_credit_activity(df):
    """
    - pos_cash_balance_NUM_POS_LOANS
    - pos_cash_balance_NUM_ACTIVE_POS_LOANS
    - pos_cash_balance_NUM_COMPLETED_POS_LOANS
    """
...
def compute_pos_cash_time_features(df):
    """
    - pos_cash_balance_AVG_POS_CREDIT_DURATION
    - pos_cash_balance_TIME_SINCE_LAST_POS_UPDATE
    - pos_cash_balance_AVG_REMAINING_INSTALLMENT_TIME
    """
    df_grp = df.groupby("SK_ID_CURR").agg(
...
def compute_pos_cash_overdue_features(df):
    """
    - pos_cash_balance_NUM_OVERDUE_POS_CREDITS (SK_DPD > 0)
    - pos_cash_balance_TOTAL_OVERDUE_DAYS_POS (sum of SK_DPD)
    - pos_cash_balance_MAX_OVERDUE_DAYS_POS (max of SK_DPD)
    - pos_cash_balance_NUM_SEVERE_OVERDUE_POS (SK_DPD_DEF > 0)
    """
    df_grp = df.groupby("SK_ID_CURR").agg(
...
def compute_pos_cash_balance_categorical_features(df):
    """
    - pos_cash_balance_PERCENT_ACTIVE_MONTHS
    - pos_cash_balance_MOST_COMMON_CONTRACT_STATUS
    - pos_cash_balance_HAS_RETURNED_CONTRACT
    """
    df_grp = df.groupby("SK_ID_CURR").agg(
